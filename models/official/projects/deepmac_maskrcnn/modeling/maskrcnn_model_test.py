...
def construct_model_and_anchors(image_size, use_gt_boxes_for_masks, bpa):
  num_classes = 3
  min_level = 2 # 3
  max_level = 6 # 4
  num_scales = 3
...
class MaskRCNNModelTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_forward(self, num, use_gt_boxes_for_masks, training, bpa):
    ...
    model, anchor_boxes = construct_model_and_anchors(
        image_size, use_gt_boxes_for_masks, bpa)
  ...
  def ttest_image_and_boxes(self, batch_size, num_boxes):
    ...
    model, _ = construct_model_and_anchors(
        image_size, use_gt_boxes_for_masks=True, bpa=False)
